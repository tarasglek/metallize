...
def main(dry_run:bool, input_tar, output_diskimage, kernel_boot_params):
    ...
    cmds = [
        ...
        f"mkdir -p {squashfs_file.parent} {isolinux_path} {grub_path} {grub_search.parent}",
        ...
        f"cp /etc/isolinux.cfg /usr/lib/ISOLINUX/isolinux.bin /usr/lib/syslinux/modules/bios/ldlinux.c32 {isolinux_path}",
        ...
        (
	        f"tar --show-transformed-names --transform='s:-.*::' --transform='s:.*/::' -xvf {input_tar} -C {isolinux_path} "
